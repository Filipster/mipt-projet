...
cv::Mat ResizeImage(cv::Mat img, int height = 800)
{
	...
	resize(img, dst, dst.size(), 0.75, 0.75, cv::INTER_AREA);
	return dst;
}
...
int FindPositionMin(float _pointer[], int length)
{
	int vt = 0;
	float min = _pointer[0];
...
int FindPositionMax(float _pointer[], int length)
{
	int vt = 0;
	float max = _pointer[0];
	for (int i = 1; i < length; i++)
	{
		if (_pointer[i] > max)
		{
...
std::vector<cv::Point2f> CornerSettings(std::vector<cv::Point2f> points)
{
...
cv::Mat DocumentDetectorImage(cv::Mat src, cv::Point2f offset)
{
	...
	roi_corners = CornerSettings(roi_corners);
...
std::vector<std::vector<cv::Point2f>>  TranformContours(std::vector<std::vector<cv::Point2f>> contours)
{
	...
	for (size_t i = 0; i < contours.size(); i++)
	{
		temp[i] = CornerSettings(contours[i]);
	}
	return temp;
}
...
{
	std::vector<cv::Point2f> dst;
	std::vector<cv::Point2f> corners = CornerSettings(src);
...
{
	cv::Mat subImage = src(cv::Rect(center.x - radius, center.y - radius, 2 * radius, 2 * radius)).clone();
	return subImage;
}
std::vector<cv::Mat> SeriImage(cv::Mat src, std::vector<std::vector<cv::Point2f>> rects)
{
	...
	for (size_t i = 0; i < rects.size(); i++)
	{
		std::vector<cv::Point2f> cornners = CornerSettings(rects[i]);
